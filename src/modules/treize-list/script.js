...
Paris.newsList = (function(){
  ...
  function newsList(selector, userOptions){
    ...
    function init(){
      ...
      $date =  $('.treize-list-agenda-nav-month'); 
      $arrondissements = $('.treize-list-agenda-arrondissement');
      ...
      // Filters action
      $('.treize-list-agenda-filters select').change(function () {
        $( ".treize-list-agenda-filters select option:selected" ).each(function() {
          if($(this).val() == 'arr'){
            $date.hide();
    ...
    function initHaveActus() {
      $('.agenda .treize-list-card-item').each(function(){
        var date = $(this).find('.treize-card-date span').text();
        date = date.split('/');
        var arr = $(this).find('.treize-card-arrondissement span').text();
        $('.treize-list-agenda-arrondissement-change').eq(arr-1).addClass('have-actus');
        $(".treize-list-agenda-nav-month span[data-month="+parseInt(date[1])+"][data-year="+parseInt(date[2])+"]").addClass('have-actus');
      });
    }
    ...
    function arrFilters(arr) {
      ...
      if(that.hasClass('have-actus')) {
        if (that.hasClass('active')) {
          ...
          $('.treize-list-agenda-arrondissement-change').removeClass('active');
          ...
          $('.agenda .treize-list-card-item').each(function(){
            ...
            if(parseInt(a) == parseInt(arr)) {
              $('.agenda-clone').append('<li class="treize-list-card-item">' + $(this).html() + '</li>');
            }
          });
        }
      }
    ...
    function monthFilters(month) {
      ...
      if(that.hasClass('have-actus')) {
        if (that.hasClass('active')) {
          ...
          $('.treize-list-agenda-nav-month span').removeClass('active');
          that.addClass('active');
...
$(document).ready(function(){
  Paris.newsList('.treize-list-agenda');
});
